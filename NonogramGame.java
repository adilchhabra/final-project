public class NonogramGame extends PuzzleGame{
	...
	public NonogramGame(Puzzle fr) { //one parameter constructor
		theFrame = fr;  
    }
   ...
   /* public void solve() {
	...
	public boolean checkConstraints(int [][] theGrid) {
		for(int i = 0; i< theGrid.length; i++) {
			...
			if(count1 >1 || count2 >1 || count3 >1 || count4 >1 || count1 <1 || count2 <1 || count3 <1 || count4 <1)
				return false;
		}
		for(int j = 0; j< theGrid.length; j++) {
			int count1 =0, count2 = 0, count3 = 0, count4 = 0;
    ...
    public boolean checkPossibleConstraints(int [][] theGrid) {
		...
		if(Math.abs(theGrid[0][1]-theGrid[1][1])<1 || Math.abs(theGrid[0][1]-theGrid[1][1])<1) {
			return false;
		}
		//if(theGrid[0][2]/theGrid[0][3]!=2 || theGrid[0][3]/theGrid[0][2]!=2) {
		//	return false;
